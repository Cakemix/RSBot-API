...
public class Quests {
	...
	public static ArrayList<Quest> getQuests() {
		...
		for (WidgetChild widgetChild : widgetChilds) {
			...
			if (widgetChild == null
					...
					|| (widgetChildText.trim().isEmpty() || widgetChild
							.getTextColor() == 16750848))
				continue;
	...
	public static boolean isNotStarted(String questName) {
		...
		for (Quest quest : quests)
			if (quest.getName().equalsIgnoreCase(questName)
					&& quest.getProgress().equals(Progress.NOT_STARTED))
				return true;
		return false;
	}
	...
	public static boolean isInProgress(String questName) {
		...
		for (Quest quest : quests)
			if (quest.getName().equalsIgnoreCase(questName)
					&& quest.getProgress().equals(Progress.IN_PROGRESS))
				return true;
		return false;
	}
	...
	public static boolean isCompleted(String questName) {
		...
		for (Quest quest : quests)
			if (quest.getName().equalsIgnoreCase(questName)
					&& quest.getProgress().equals(Progress.COMPLETED))
				return true;
		return false;
	}
